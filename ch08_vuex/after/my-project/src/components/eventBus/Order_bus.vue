...
export default {
    ...
    created() {
        this.$eventBus.$off('chgDiscAmt');
        this.$eventBus.$on('chgDiscAmt', this.setCharge);
    },
    methods: {
        calcTransFee() {
            this.isCalc = true;
        ...
        promotion() {
            ...
            this.$eventBus.$emit('changePromotion', {isPromotion: this.isPromotion, amount: this.promotionAmount});
        },
        order() {
            if(!this.isCalc) {
                alert('배송비를 계산해주세요.');
